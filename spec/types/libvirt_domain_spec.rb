...
describe 'libvirt_domain' do
  on_supported_os.each do |os, os_facts|
    context "on #{os}" do
      let(:facts) { os_facts }

      let(:title) { 'libvirt-domain' }
      ...
      context 'with default provider' do
        ...
        it { is_expected.to be_valid_type.with_parameters('replace') }
        it { is_expected.to be_valid_type.with_parameters('ignore') }
      end
    end
  end
end
